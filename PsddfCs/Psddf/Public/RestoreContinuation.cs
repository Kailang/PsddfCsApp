namespace PsddfCs {
	public partial class Psddf {
		...
		/// </summary>
		public void RestoreContinuation (string path) {
			Io.OpenRead(INS, path);
			...
			StartPrintTime = Io.StreamReadInt(INS);
			...
			*/
			for (int i = 1; i <= DredgedFillLayers; i++) {
				...
				for (int j = 1; j <= RelationDefinitionLines[id]; j++) {
					// read(ins) voidratio(k, id), perm(k, id), effectivestress(k, id)
					VoidRatios[j, id] = Io.StreamReadDouble(INS);
					Permeabilities[j, id] = Io.StreamReadDouble(INS);
					EffectiveStresses[j, id] = Io.StreamReadDouble(INS);
				}
			}
    ...
    if (nbl /= 2) then ! Check for Compressible Foundation
        ...
        enddo
			...
			*/
			...
			if (!Io.EndOfFile(INS))
				throw new System.Exception("Continuation file is corrupted");
